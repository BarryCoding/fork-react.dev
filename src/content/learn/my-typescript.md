...
These sandboxes can handle TypeScript code, but they **do not run the type-checker**. This means you can amend the TypeScript sandboxes to learn, but **you won't get any type errors or warnings**.
...
If you want to explicitly provide a type for the state:
...
a common case where you may want to provide a type is when you have a union type.
...
Or, as recommended in [Principles for structuring state](/learn/my-choosing-the-state-structure#principles-for-structuring-state), group related state as an object and describe the different possibilities via object types:
...
This technique works when you have a **default value** which makes sense - but there are occasionally cases when you do not, and in those cases `null` can feel reasonable as a default value.  
However, to allow the type-system to understand your code, you need to explicitly set `ContextShape | null` on the `createContext`. 
...
There is quite an expansive set of types which come from the `@types/react` package [in React's folder in DefinitelyTyped](https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/react/index.d.ts), it is worth a read.

### DOM Events {/*typing-dom-events*/}

when you want to extract a function to be passed to an event handler, you will need to explicitly set the type of the event.
